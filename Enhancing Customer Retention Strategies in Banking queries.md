# Enhancing Customer Retention Strategies in Banking_USING_SQL
...
```sql
select count(*) as TOTAL_RECORDS from churn;
```
...
```SQL
ALTER TABLE churn
ADD credit_score_category VARCHAR(10);
...
SELECT 
    country,
    COUNT(CASE WHEN churn = 1 THEN 1 END) AS tot_churn_customer,
    COUNT(CASE WHEN churn = 0 THEN 1 END) AS tot_non_churn_customer
FROM churn
GROUP BY country
ORDER BY tot_churn_customer DESC;
```
...
SELECT 
    ...
    tot_churn_customer,
    tot_customer,
    CONCAT(ROUND((tot_churn_customer * 100.0 / tot_customer), 1), ' %') AS churn_rate
FROM (
    SELECT 
        products_number,
        COUNT(CASE WHEN churn = 1 THEN 1 END) AS tot_churn_customer,
        COUNT(*) AS tot_customer
    FROM churn
    GROUP BY products_number
) AS x;
```
...
SELECT 
    active_member,
    CONCAT(ROUND((tot_churn_customer * 100.0 / tot), 2), '%') AS churn_rate
FROM (
    SELECT 
        active_member,
        COUNT(CASE WHEN churn = 1 THEN 1 END) AS tot_churn_customer,
...
SELECT 
    credit_score_category,
    tot,
    ROUND(tot * 100.0 / SUM(tot) OVER (), 2) AS percentage
FROM churn_data
ORDER BY tot DESC;
```
<img width="218" alt="Screenshot 2024-03-14 102142" src="https://github.com/nitish4393/BANK_CUSTOMER_CHURN_ANALYSIS_USING_SQL/assets/120879393/0ce8718f-2386-44c2-9c86-b45c929f3085">
  ...
  - INSIGHT
     ...
     - The distribution of churned customers by credit score category reveals a notable concentration among 'Fair', 'Poor', and 'Good' credit score categories
     - Customers with lower credit scores ('Fair' and 'Poor') demonstrate a higher propensity to churn compared to those with higher credit scores ('Very Good' and 'Excellent').
     
 # PREDICTION 
 
  - Out of the total 7,963 customers, an estimated 31% categorized under 'Fair', 'Poor', and 'Good' credit score categories, who are currently inactive, are at risk of churning.
     
 # RECOMMENDATIONS
   1. Focus on Customer Engagement

     -Implement personalized incentives and targeted promotions to      re-engage inactive members.
     -Enhance customer support to improve satisfaction and retention.

   2. Improve Product Offerings

     -Investigate reasons behind high churn in product categories 3 and 4.
     -Conduct market research to align offerings with customer expectations.

   3. Targeted Marketing and Retention Strategies

     -Develop segmented marketing campaigns based on credit score categories, age groups, and gender.
     -Offer tailored products and services to meet specific customer needs.

   4. Enhance Customer Service Quality

     -Invest in training programs to elevate service standards.
     -Establish feedback mechanisms for continuous improvement.

   5. Monitor and Address Churn Contributors

     -Proactively address factors influencing churn through targeted interventions and loyalty programs.
     -Conduct surveys to understand customer pain points and preferences.

   6. Expand Market Reach

     -Explore opportunities to expand into new regions with localized strategies.
     -Develop region-specific marketing initiatives to attract and retain customers.
